...
typedef struct {
    ...
    int  nargs;
    int  defIdx;   /* index of the SUBROUTINE/FUNCTION statement itself */
    int  bodyIdx;  /* index of the first statement inside it */
    int  endIdx;   /* index of its END statement */
    int  isFunc;   /* 1 for an external FUNCTION (callable from an
                       expression, returning a value via its own name),
                       0 for a SUBROUTINE (invoked only via CALL) */
} SUBPROG;
...
/* Per-line cache for assignment statements. A given source line's
   "shape" (plain scalar assignment, array-element assignment, or
   something else entirely -- a keyword statement or a statement
   function definition) is fixed once execution has confirmed it,
   since that determination depends only on the line's own static
   text and on vars[]/stmtfuncs[] state that, for a name once
   resolved, never reverts. Caching it lets a loop body's assignment
   statement -- the single most common statement type inside a tight
   loop -- skip the entire ~20-keyword dispatch chain and the target
   variable's name lookup on every iteration after the first, paying
   that cost exactly once per line rather than once per execution.
   The subscript and right-hand-side expressions are still parsed
   and evaluated fresh every time, since their values genuinely do
   change from one iteration to the next -- only the fixed,
   iteration-independent parts of the statement's shape are cached.
   name[] is cached separately from target->name because those
   differ for an aliased subroutine parameter (the formal parameter
   name written in the source text vs. the caller's actual variable
   the alias points to), and trace output must reflect what was
   actually written, not what it happens to alias to. */
typedef struct {
    int   valid;      /* 1 once this line has been confirmed a plain assignment */
    int   isArraySub; /* 1 if NAME(subs) = expr, 0 if NAME = expr */
    VAR  *target;      /* resolved target variable, valid because vars[] is a fixed array that never relocates */
    char  name[8];     /* the source-text name itself, for correct trace output */
    int   restOffset; /* offset into prog[idx].text where parsing resumes: right at '(' for an array assignment, or right at '=' for a scalar one */
} ASSIGNCACHE;
static ASSIGNCACHE FARBIG asgnCache[MAXLINES];
...
static VAL callUserFunction(SUBPROG *sp, char **p);
static long packStrBits(const char *s);
static float packStrBitsAsReal(const char *s);
...
static VAR *findvar(const char *name)
{
    ...
    char c0 = name[0];
    for (i = 0; i < nvars; i++)
        if (vars[i].name[0] == c0 && strcmp(vars[i].name, name) == 0) return &vars[i];
    return NULL;
}
...
static VAR *findvarAliased(const char *name)
{
    if (ncallstack > 0) {
        ...
        for (i=0;i<cf->nbinds;i++)
            if (cf->binds[i].formalName[0] == c0 && !strcmp(cf->binds[i].formalName, name)) return cf->binds[i].actual;
    }
    return findvar(name);
}
...
static int isIntrinsic(const char *nm)
{
    ...
    char c0 = nm[0];
    for (i = 0; list[i]; i++)
        if (list[i][0] == c0 && strcmp(list[i], nm) == 0) return 1;
    return 0;
}
...
static VAL callIntrinsic(const char *nm, VAL *a, int na)
{
    ...
    if (!strcmp(nm,"REAL"))   return mkreal(VF(a[0]));
    if (!strcmp(nm,"NINT"))   { double x=(double)VF(a[0]); return mkint(x>=0.0 ? (long)(x+0.5) : -(long)(-x+0.5)); }
...
static VAL parsePrimary(char **p)
{
    ...
    if (**p == '(') {
        (*p)++;
        /* evalLogical falls straight through to plain arithmetic when no
           relational/logical operator is present, so this one change
           covers both ordinary arithmetic grouping, e.g. (2.*SLICES), and
           grouped relational/logical sub-expressions, e.g. (B.GE.5), with
           no change in behavior for the arithmetic-only case. */
        v = evalLogical(p);
        skipsp(p);
    ...
    if (**p=='.') {
        if (matchDot(p,"TRUE"))  { emitOp("LIT","1"); return mkint(1); }
        if (matchDot(p,"FALSE")) { emitOp("LIT","0"); return mkint(0); }
    }
    ...
    if (isnamestart((unsigned char)**p)) {
        ...
        if (**p=='(') {
            ...
            {
                SUBPROG *sp = findSubprog(nm);
                if (sp && sp->isFunc) {
                    return callUserFunction(sp, p);
                }
            }
            {
...
/* Emulates INTEGER (16-bit signed, two's-complement) overflow
   wraparound -- the width this interpreter's original 16-bit DOS/
   Turbo C target actually stored FORTRAN INTEGERs in. Classic idioms
   of that era (e.g. a linear-congruential RNG seed update like
   IY = IY*899, with a follow-up IF (IY.LT.0) fixup) depend on
   arithmetic genuinely overflowing and wrapping at 16 bits rather
   than silently widening the way a host `long` otherwise would.
   Scoped to the arithmetic operators themselves (+, -, *, and integer
   **), matching where a real 16-bit ALU would produce a truncated
   result -- not to storage, I/O, or intrinsics, which is a separate
   and much broader question this doesn't need to answer. */
static long wrap16(long v)
{
    unsigned long u = (unsigned long)v & 0xFFFFUL;
    if (u & 0x8000UL) return (long)u - 0x10000L;
    return (long)u;
}
...
static VAL binop(char op, VAL a, VAL b)
{
    ...
    VAL result;
    if (op=='+') result = real ? mkreal(VF(a)+VF(b)) : mkint(wrap16(VI(a)+VI(b)));
    else if (op=='-') result = real ? mkreal(VF(a)-VF(b)) : mkint(wrap16(VI(a)-VI(b)));
    else if (op=='*') result = real ? mkreal(VF(a)*VF(b)) : mkint(wrap16(VI(a)*VI(b)));
    else if (op=='/') {
...
static VAL parsePower(char **p)
{
    ...
    if ((*p)[0]=='*' && (*p)[1]=='*') {
        ...
        if (!exp.isReal && exp.i>=0 && !base.isReal) {
            long r=1, b=base.i, n=exp.i; while(n-->0) r=wrap16(r*b); result = mkint(r);
        } else {
...
{
    char *s; const char *w;
    if ((*p)[0] != '.') return 0; /* cheap check first -- rules out the vast majority of calls before touching word at all */
    s = *p + 1;
    w = word;
    while (*w) {
        if (toupper((unsigned char)*s) != (unsigned char)*w) return 0;
        s++; w++;
    }
    if (*s != '.') return 0;
    *p = s + 1;
...
static void putCharToBuf(char *dst, int clen, VAL v)
{
    ...
    int i, srclen;
    if (v.isStr) src = v.s;
    else if (v.isReal) { sprintf(numbuf, "%g", (double)v.r); src = numbuf; }
    else { sprintf(numbuf, "%ld", v.i); src = numbuf; }
    srclen = (int)strlen(src);
    for (i=0; i<clen; i++) dst[i] = (i<srclen) ? src[i] : ' ';
}

static void storeIntoVar(VAR *var, const char *displayName, int hasSub, long i1,long i2,long i3, VAL v)
{
    if (hasSub) {
        long idx;
        ...
        else if (var->type==0) var->iarr[idx] = v.isStr ? packStrBits(v.s) : VI(v);
        else var->rarr[idx] = v.isStr ? packStrBitsAsReal(v.s) : VF(v);
        emitOp("STOX", displayName);
    } else {
        if (var->isArray) { fatal("syntax error"); return; }
        if (var->type==2) putCharToBuf(var->cval, var->clen, v);
        else if (var->type==0) var->ival = v.isStr ? packStrBits(v.s) : VI(v);
        else var->rval = v.isStr ? packStrBitsAsReal(v.s) : VF(v);
        emitOp("STO", displayName);
    }
}

static void storeInto(char *name, int hasSub, long i1,long i2,long i3, VAL v)
{
    storeIntoVar(getvar(name), name, hasSub, i1,i2,i3, v);
}
...
/* Packs up to the first 4 bytes of a Hollerith-style string constant
   into a plain integer bit pattern (byte i in bits [8i..8i+7]). For a
   1-character string this reduces to exactly that character's own
   code -- identical to the single-char Hollerith value this
   interpreter already stored, so it changes nothing for that
   already-working case (e.g. graph.for's DASH/EYE/SPACE/STAR); it
   only extends correctly to something like maze.for's EDD/EBB/BBI/
   BBB wall-segment text, each packed into a REAL (the wider, 4-byte
   word on this interpreter's 16-bit-INTEGER original target -- see
   wrap16() above) and printed back with an A3 edit descriptor. */
static long packStrBits(const char *s)
{
    long v = 0; int i, n = (int)strlen(s); if (n > 4) n = 4;
    for (i = 0; i < n; i++) v |= ((long)(unsigned char)s[i]) << (i*8);
    return v;
}

/* Same packing, reinterpreted directly as a REAL's raw 4-byte storage
   rather than converted through its numeric value -- exactly how
   packing text into a REAL word works on real hardware: the bytes
   are whatever they are, regardless of what floating-point number
   they'd form if read back as a value. */
static float packStrBitsAsReal(const char *s)
{
    union { float f; unsigned char b[4]; } u;
    int i, n = (int)strlen(s); if (n > 4) n = 4;
    u.f = 0.0f;
    for (i = 0; i < n; i++) u.b[i] = (unsigned char)s[i];
    return u.f;
}

/* Inverse of the two packers above: unpacks up to 4 characters back
   out of a plain integer or REAL's raw bits into out[], stopping at
   the first zero byte -- so a single packed character round-trips to
   exactly that one character, matching prior behavior exactly.
   Returns the unpacked length. */
static int unpackIntBits(long v, char *out)
{
    int i;
    for (i = 0; i < 4; i++) {
...
static int unpackRealBits(float f, char *out)
{
    union { float f; unsigned char b[4]; } u;
    int i;
    u.f = f;
...
static void putAField(VAL v, int w)
{
    ...
    if (v.isStr) { strncpy(buf, v.s, sizeof(buf)-1); buf[sizeof(buf)-1]=0; len=(int)strlen(buf); }
    else if (v.isReal) { len = unpackRealBits(v.r, buf); }
    else { len = unpackIntBits(v.i, buf); }
...
{
    while (*kw) {
        if (toupper((unsigned char)*s) != (unsigned char)*kw) return 0;
        s++; kw++;
    }
    /* next char must not be a name char, so INTEGERX isn't matched as INTEGER */
    if (isnamechar((unsigned char)*s)) return 0;
    return 1;
}
...
{
    int i;
    for (i=0;i<nfmts;i++) if (fmts[i].label==lab) return fmts[i].text;
    return NULL;
}
...
static void doData(char *p)
{
    for (;;) {
        ...
        for (;;) {
            VAL v;
            skipsp(&p);
            /* optional "N*" repeat-count prefix, e.g. DATA TOP/33*'_'/ --
               consumed and discarded rather than expanded into N stored
               slots: round-robin cycling over a single stored value
               already replicates it correctly whenever the count matches
               (or evenly divides) the target's size, which covers every
               real use of this syntax without growing these stack
               buffers (they're shared with real-hardware Turbo C builds
               with limited stack). */
            {
                char *look = p; char rb[16]; int rn = 0;
                while (isdigit((unsigned char)*look) && rn < 15) rb[rn++] = *look++;
                if (rn > 0 && *look == '*' && look[1] != '*') p = look + 1;
                skipsp(&p);
            }
            v = parseUnary(&p);
        ...
        for (i=0;i<nn;i++) {
            ...
            if (var->isArray) {
                ...
                for (k=0;k<var->size;k++) {
                    ...
                    else if (var->type==0) var->iarr[k] = vv.isStr ? packStrBits(vv.s) : VI(vv);
                    else var->rarr[k] = vv.isStr ? packStrBitsAsReal(vv.s) : VF(vv);
                }
            } else {
...
static void doCharacterDecl(char *p)
{
    ...
    if (*p=='*') {
        p++; skipsp(&p);
        if (*p=='(') {
            /* CHARACTER*(expr) -- length given by a parenthesized
               expression, typically a PARAMETER-declared named
               constant (e.g. CHARACTER*(HPOS)); PARAMETER statements
               execute in ordinary program order, so by the time this
               declaration itself executes, the name is already a
               plain variable holding its value, readable like any
               other. */
            VAL v; p++; v = parseExprTop(&p); skipsp(&p);
            if (*p==')') p++;
            deflen = (int)VI(v);
        } else {
            char nb[16]; int n=0;
...
static void doAssignOrCall(char *p, char *stmtBase, int idx)
{
    char nm[8]; char *save;
    skipsp(&p);
    save = p;
    ...
    if (*p=='(') {
        ...
        {
        VAL subs[3]; int ns; long i1=1,i2=1,i3=1; VAL rhs; VAR *var; char *parenAt;
        var = getvar(nm);
        parenAt = p; /* p is sitting right on '(' here, before parseArgList consumes it */

        /* CHARACTER substring assignment target: NAME(i:j) = expr, on a
           scalar (non-array) CHARACTER variable -- a distinct syntax
           from array-element subscripting (comma-separated), detected
           here by peeking for a ':' before falling back to the normal
           parseArgList path below for ordinary array-element targets. */
        if (var->type==2 && !var->isArray) {
            char *q2 = p + 1; VAL a1;
            skipsp(&q2);
            a1 = parseExprTop(&q2);
            skipsp(&q2);
            if (*q2==':') {
                VAL a2; long start, end;
                q2++;
                a2 = parseExprTop(&q2);
                skipsp(&q2);
                if (*q2!=')') { fatal("syntax error"); return; }
                q2++;
                skipsp(&q2);
                if (*q2!='=') { fatal("syntax error"); return; }
                q2++;
                rhs = parseConcatExpr(&q2);
                start = VI(a1); end = VI(a2);
                if (start<1) start=1;
                if (end>var->clen) end=var->clen;
                {
                    char numbuf[32]; const char *src; int srclen, i, di;
                    if (rhs.isStr) src = rhs.s;
                    else if (rhs.isReal) { sprintf(numbuf, "%g", (double)rhs.r); src = numbuf; }
                    else { sprintf(numbuf, "%ld", rhs.i); src = numbuf; }
                    srclen = (int)strlen(src);
                    di = 0;
                    for (i=(int)start; i<=(int)end; i++) {
                        if (i>=1 && i<=var->clen) var->cval[i-1] = (di<srclen) ? src[di] : ' ';
                        di++;
                    }
                }
                emitOp("STOSUB", nm);
                if (idx >= 0) asgnCache[idx].valid = 0; /* substring targets aren't statement-cached */
                return;
            }
        }
        ...
        storeIntoVar(var, nm, 1, i1,i2,i3, rhs);
        if (idx >= 0) {
            asgnCache[idx].valid = 1;
            asgnCache[idx].isArraySub = 1;
            asgnCache[idx].target = var;
            strncpy(asgnCache[idx].name, nm, 7); asgnCache[idx].name[7] = 0;
            asgnCache[idx].restOffset = (int)(parenAt - stmtBase);
        }
        return;
        }
    }
    skipsp(&p);
    if (*p=='=') {
        VAL rhs; VAR *var; char *eqAt = p;
        p++;
        rhs = parseConcatExpr(&p);
        var = getvar(nm);
        storeIntoVar(var, nm, 0, 0,0,0, rhs);
        if (idx >= 0) {
            asgnCache[idx].valid = 1;
            asgnCache[idx].isArraySub = 0;
            asgnCache[idx].target = var;
            strncpy(asgnCache[idx].name, nm, 7); asgnCache[idx].name[7] = 0;
            asgnCache[idx].restOffset = (int)(eqAt - stmtBase);
        }
        return;
    }
    fatal("syntax error");
...
static void doRead(char *p)
{
    ...
    if (*p=='(') {
        ...
        /* READ *, list -- the common bare list-directed form; parseUnitFmt
           only understands the parenthesized (unit,fmt) form above, so
           this bare form needs its own prefix handling, mirroring how
           doWriteOrPrint already handles PRINT's identical bare syntax. */
        listDirected = 1; lab = 0;
        p++;
        skipsp(&p);
        if (*p==',') p++;
    } else {
    ...
    lp = p;
    skipsp(&lp);
    /* simple: read one value per named variable/array-ref, list-directed from stdin */
    for (;;) {
        char nm[8]; VAR *var;
        skipsp(&lp);
        ...
        if (*lp=='(') {
            ...
            if (ns>=3) i3=VI(subs[2]);
            idx = arrIndex(var,i1,i2,i3);
            if (var->type==0) { long t; scanf("%ld",&t); var->iarr[idx]=t; }
            else { float t; scanf("%f",&t); var->rarr[idx]=t; }
        } else if (var->isArray) {
            /* bare array name, no subscript: reads its whole element
               sequence in order -- the standard list-directed-READ
               meaning of naming an array with no subscript. */
            long k;
            for (k=0; k<var->size; k++) {
                if (var->type==0) { long t; scanf("%ld",&t); var->iarr[k]=t; }
                else { float t; scanf("%f",&t); var->rarr[k]=t; }
            }
        } else {
            if (var->type==0) scanf("%ld", &var->ival);
            else scanf("%f", &var->rval);
        }
        skipsp(&lp);
        if (*lp==',') { lp++; continue; }
        break;
...
static void doGoto(char *p)
{
    ...
    skipsp(&p);
    if (*p=='(') {
        /* computed GOTO: GOTO (l1,l2,...,lN), intexpr -- jumps to the
           selector'th label, 1-based; an out-of-range selector falls
           through to the next statement, per the FORTRAN standard. */
        long labs[64]; int nl=0; VAL v; long sel;
        p++;
        for (;;) {
            skipsp(&p);
            n=0;
            ...
            for (;;) {
                ...
                if (!actual) {
                    ...
                    if (v.isStr) {
                        ...
                        memset(scratchbuf[ncallstack][slot], ' ', 127);
                        scratchbuf[ncallstack][slot][127] = 0;
                        memcpy(scratchbuf[ncallstack][slot], v.s, vlen);
                        tmp->clen = (int)vlen;
                        tmp->cval = scratchbuf[ncallstack][slot];
...
/* Invokes an external FUNCTION subprogram from within expression
   evaluation and returns its result. Unlike CALL (a full statement,
   which just jumps pc into the subprogram and lets the single
   top-level runFrom() loop carry on until RETURN/END pops back), this
   is reached from deep inside expression parsing and needs a value
   back immediately -- so it drives its own small nested loop over
   just the function's body, using the exact same execIndex()/
   doReturn() machinery runFrom() itself uses, confined to this call's
   own call-stack frame rather than resetting any shared state the way
   a fresh runFrom() call would.
   Argument binding mirrors doCall() exactly: a bare variable name
   (RND(IX)) is a true alias, so the callee can mutate the caller's own
   variable -- classic FORTRAN idioms rely on this (e.g. a
   linear-congruential RNG that advances its own seed argument by
   reference on every call, as maze.for's RND does). Anything else
   (a literal, an expression, a subscripted element) is evaluated once
   into a private scratch variable, since there's no single caller
   variable to alias.
   *p must point at the function name's trailing '(' (or at whatever
   follows the name, if it takes no arguments) on entry; it is
   advanced past the closing ')' before returning. */
static VAL callUserFunction(SUBPROG *sp, char **p)
{
    CALLFRAME *cf;
    int savedNcallTarget;
    int outerPc = pc;
    VAR *retvar;
    VAL result;

    if (ncallstack >= MAXCALLDEPTH) { fatal("out of memory"); return mkint(0); }
    cf = &callstack[ncallstack];
    cf->nbinds = 0;
    ...
    if (**p=='(') {
        ...
        if (**p != ')') {
            for (;;) {
                ...
                if (!actual) {
                    /* anything else (literal, expression, subscripted
                       element): evaluate once into a scratch var --
                       writes inside the callee won't propagate back,
                       since there's no single element to alias */
                    VAL v = parseConcatExpr(p);
                    int slot = cf->nbinds < MAXSUBARGS ? cf->nbinds : 0;
                    VAR *tmp = &scratchvars[ncallstack][slot];
                    tmp->isArray = 0; tmp->type = v.isStr ? 2 : (v.isReal ? 1 : 0);
    ...
    {
        long guard = 0;
        while (running && ncallstack > savedNcallTarget) {
            pc++;
            if (pc < 0 || pc >= nlines) { fatal("function fell off end"); break; }
            if (!prog[pc].used) continue;
            execIndex(pc);
            if (++guard > 2000000L) { fatal("function did not return"); break; }
        }
    }

    /* The function's result is whatever the body last stored in the
       variable matching its own name -- the standard FORTRAN
       convention (set exactly like an ordinary assignment inside the
       body, e.g. "F = expr"). Read it back fresh via getvar() since a
       parameter alias could in principle share that name. */
    retvar = getvar(sp->name);
    if (retvar->type==2) result = mkstr(retvar->cval ? retvar->cval : "");
    else if (retvar->type==0) result = mkint(retvar->ival);
    else result = mkreal(retvar->rval);

    pc = outerPc;
    return result;
}

static void execOneStatement(char *stmt, int idx); /* fwd -- idx is the line index in prog[] for caching, or -1 if not cacheable (nested-IF body, immediate-mode REPL statement) */

/* ------------------------------------------------------------------
   block IF / ELSE IF / ELSE / END IF
   ------------------------------------------------------------------ */

/* Classifies one program line for IF-block matching purposes, WITHOUT
   evaluating or executing it: 0=ordinary statement, 1=opens a block IF
   ("IF (...) THEN", with THEN as the entire rest of the line -- this is
   what distinguishes it from an ordinary single-statement logical IF,
   which does not affect nesting), 2=ELSE IF, 3=ELSE, 4=END IF (either
   "ENDIF" or "END IF"). condOut, if non-NULL, is set (for kinds 1 and 2)
   to point at the condition's opening '(', ready for evalLogical --
   which itself handles the parenthesized grouping via the paren-primary
   fix above, so no manual paren-skipping is needed here. */
static int classifyIfStmt(char *stmt, char **condOut)
{
    char *p = stmt;
    skipsp(&p);
    if (kwis(p,"ELSE")) {
        ...
        if (kwis(q,"IF")) {
            char *r = q+2;
            skipsp(&r);
            if (condOut) *condOut = r;
            return 2;
        }
        if (*q==0) return 3;
        return 0;
    }
    if (kwis(p,"ENDIF")) return 4;
    if (kwis(p,"END")) {
        char *q = p+3;
        skipsp(&q);
        if (kwis(q,"IF")) return 4;
        return 0;
    }
    if (kwis(p,"IF")) {
        char *q = p+2; int depth;
        skipsp(&q);
        if (*q!='(') return 0;
        if (condOut) *condOut = q;
        depth = 0;
        for (;;) {
            if (*q=='\'') { q = (char*)scanQuotedString(q, NULL, 0); continue; }
            if (*q=='(') depth++;
...
/* Scans forward from startIdx for the END IF that closes whatever block
   IF is (or contains) startIdx, skipping past any ELSE/ELSE IF markers
   along the way -- used when a chosen branch's body has just finished
   executing and control reaches the next marker of the SAME construct,
   which must always mean "done, skip anything else in this construct". */
static int findConstructEnd(int startIdx)
{
    int depth = 0;
    int i;
    for (i = startIdx; i < nlines; i++) {
        int k;
        if (!prog[i].used) continue;
        k = classifyIfStmt(prog[i].text, NULL);
        if (k == 1) { depth++; continue; }
        if (k == 4) { if (depth == 0) return i; depth--; continue; }
    }
    return -1;
}

/* Called when a block IF's own condition (or an ELSE IF's) has just
   evaluated false. Scans forward from startIdx for the next thing that
   should run: a true ELSE IF branch (evaluating each candidate's own
   condition along the way), an unconditional ELSE, or -- if nothing
   matches -- the construct's END IF itself. Returns the index of the
   next statement to DISPATCH (a body statement for a taken branch, or
   the END IF marker line if nothing was taken), or -1 for malformed
   source (IF...THEN with no matching END IF). */
static int resolveIfFalse(int startIdx)
{
    int depth = 0;
    int i = startIdx;
    while (i < nlines) {
        int k; char *condp = NULL;
        if (!prog[i].used) { i++; continue; }
        k = classifyIfStmt(prog[i].text, &condp);
        if (k == 1) { depth++; i++; continue; }
        if (k == 4) {
            if (depth == 0) return i;
            depth--; i++; continue;
        }
        if (depth == 0 && k == 2) {
            char *q = condp; VAL cv = evalLogical(&q);
            if (VI(cv)) return i + 1;
            i++; continue;
        }
        if (depth == 0 && k == 3) return i + 1;
        i++;
    }
    return -1;
}
...
static void doIf(char *p)
{
    ...
    afterparen = q;
    skipsp(&afterparen);
    /* block IF: rest is exactly THEN */
    if (kwis(afterparen,"THEN")) {
        char *r = afterparen+4;
        skipsp(&r);
        if (*r==0) {
            emitOp("IF", "THEN");
            if (VI(cond)) {
                return; /* fall through: pc+1 begins the THEN-block */
            } else {
                int nextIdx = resolveIfFalse(pc+1);
                if (nextIdx<0) { fatal("if without matching end if"); return; }
                pc = nextIdx - 1;
                return;
            }
        }
    ...
    if (VI(cond)) {
        execOneStatement(afterparen, -1); /* nested IF body: not the top-level statement text, not cached */
    }
}
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
static void execOneStatement(char *stmt, int idx)
{
    char *p = stmt;
    skipsp(&p);
    if (*p==0) return;
    /* Fast path: if this exact line was already confirmed, on an
       earlier execution, to be a plain assignment, skip the entire
       keyword dispatch chain below and the target-name lookup --
       both would just repeat the same result they gave last time.
       This has to sit here, before any kwis() call, to actually
       save the dispatch cost; checking inside doAssignOrCall itself
       would only save the (smaller) name-lookup cost, since that's
       only reached after every keyword check has already failed. */
    if (idx >= 0 && asgnCache[idx].valid) {
        ASSIGNCACHE *ac = &asgnCache[idx];
        char *rp = stmt + ac->restOffset;
        VAL rhs;
        if (ac->isArraySub) {
            VAL subs[3]; int ns; long i1=1,i2=1,i3=1;
            ns = parseArgList(&rp, subs, 3);
            if (ns>=1) i1=VI(subs[0]);
            if (ns>=2) i2=VI(subs[1]);
            if (ns>=3) i3=VI(subs[2]);
            skipsp(&rp);
            if (*rp!='=') { fatal("syntax error"); return; }
            rp++;
            rhs = parseConcatExpr(&rp);
            storeIntoVar(ac->target, ac->name, 1, i1,i2,i3, rhs);
        } else {
            skipsp(&rp);
            if (*rp!='=') { fatal("syntax error"); return; }
            rp++;
            rhs = parseConcatExpr(&rp);
            storeIntoVar(ac->target, ac->name, 0, 0,0,0, rhs);
        }
        return;
    }
    /* Loop-relevant statement types first: every statement that
       matches none of these (an assignment, by far the most common
       statement inside a tight loop) still has to fail every check
       before it before falling through to doAssignOrCall, so the
       once-per-program declaration keywords are checked last to
       keep that failure path shorter for the common case. */
    if (kwis(p,"CONTINUE"))  { return; }
    if (kwis(p,"IF"))        { doIf(p+2); return; }
    if (kwis(p,"ELSE")) {
        /* Reached by ordinary sequential execution -- meaning whatever
           branch was taken (the THEN-block or a prior ELSE IF/ELSE) has
           just finished, so this marker (ELSE, or ELSE IF -- its own
           condition is irrelevant here, since that's only ever evaluated
           from resolveIfFalse when the governing condition was false)
           always means: skip everything else belonging to this
           construct and land on its END IF. */
        int endIdx = findConstructEnd(pc+1);
        if (endIdx<0) { fatal("if without matching end if"); return; }
        emitOp("SKIPIF", NULL);
        pc = endIdx - 1;
        return;
    }
    if (kwis(p,"ENDIF"))     { return; } /* block terminator, no-op when reached directly */
    ...
    if (kwis(p,"END")) {
        char *q2 = p+3; skipsp(&q2);
        if (kwis(q2,"IF")) { return; } /* "END IF" (with a space): block terminator, no-op */
    ...
    if (kwis(p,"LOGICAL"))   { doTypeDecl(p+7, 0); return; } /* stored as INTEGER 0/1; this interpreter has no distinct logical storage type */
    ...
    for (i=0;i<nlines;i++) {
        ...
        if (kwis(p,"FORMAT") && prog[i].used) {
            if (nfmts < MAXFORMATS) {
                ...
                strncpy(fmts[nfmts].text, p+6, LINELEN-1);
...
static void prescanSubprograms(void)
{
    ...
    for (i=0;i<nlines;i++) {
        ...
        skipsp(&p);
        {
        int isFuncDecl = 0; int kwlen = 0; char *fp = p;
        if (kwis(fp,"FUNCTION")) { isFuncDecl = 1; kwlen = 8; }
        else if (kwis(fp,"REAL")) { char *r=fp+4; skipsp(&r); if (kwis(r,"FUNCTION")) { isFuncDecl=1; fp=r; kwlen=8; } }
        else if (kwis(fp,"INTEGER")) { char *r=fp+7; skipsp(&r); if (kwis(r,"FUNCTION")) { isFuncDecl=1; fp=r; kwlen=8; } }
        if (isFuncDecl) {
            SUBPROG *sp; char *q = fp+kwlen; char nm[8]; int j;
...
/* Copies prog[idx].text into a local stack buffer before parsing it,
   rather than parsing prog[] (a far-qualified array under real Turbo
   C large-model builds) directly. On a flat-memory host this is a
   no-op distinction, but in real 8086 large-model code a stack local
   is near (SS:BP-relative), while prog[] requires far segment:offset
   arithmetic on every access -- and parsing re-reads the same text
   character by character, many times over, across many nested
   parser calls, for a single statement. If far-pointer overhead is
   the dominant real-hardware cost (plausible, and not something
   flat-memory benchmarking on this development host can detect at
   all), this is the fix that actually targets it; the one-time copy
   cost is paid once per statement rather than once per character
   read during that statement's parsing. */
static void execIndex(int idx)
{
    long lab = prog[idx].label;
    listStatementHeader(lab, prog[idx].text);
    execOneStatement(prog[idx].text, idx);
...
static void runFrom(int startIdx)
{
    ...
    { int i; for (i=0;i<MAXLINES;i++) asgnCache[i].valid = 0; }
    pc = startIdx;
    pcAddr = 0;
...
static void storeLine(long lab, const char *text)
{
    ...
    for (i=0;i<nlines;i++) {
        if (prog[i].used && prog[i].label==lab) {
            strncpy(prog[i].text, text, LINELEN-1); prog[i].text[LINELEN-1]=0;
            return;
        }
    }
    ...
    /* insert in sorted order by label */
    {
        ...
        prog[pos].label = lab;
        strncpy(prog[pos].text, text, LINELEN-1); prog[pos].text[LINELEN-1]=0;
        prog[pos].used = 1;
        nlines++;
    }
}
...
static int loadLine(const char *raw)
{
    ...
    /* immediate execution */
    execOneStatement(text, -1); /* immediate-mode REPL statement: never stored in prog[], not cached */
    return 1;
}
...
static void appendLineDirect(long lab, const char *text)
{
    ...
    strncpy(prog[nlines].text, text, LINELEN-1); prog[nlines].text[LINELEN-1]=0;
    prog[nlines].used = 1;
    nlines++;
}
...
static int loadFile(const char *fn)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (*q=='+' || *q=='>' || *q=='$' || *q=='&') {
            ...
            if (nlines > 0) {
                char *cont = q + 1;
                size_t curlen = strlen(prog[nlines-1].text);
                size_t L;
                skipsp(&cont);
                L = strlen(cont);
                while (L>0 && (cont[L-1]=='\n' || cont[L-1]=='\r' || cont[L-1]==' ')) cont[--L]=0;
                if (curlen + 1 < (size_t)(LINELEN-1)) {
                    strcat(prog[nlines-1].text, " ");
                    strncat(prog[nlines-1].text, cont, (LINELEN-2-curlen));
                }
            }
            continue;
        }
...
static void printBanner(void)
{
    ...
    printf("IV-TRAN REV. 5.21\n");
    printf("INTERACTIVE ANSI FORTRAN IV COMPILER/INTERPRETER\n");
    printf("COPYRIGHT 1966,1977,1981 (C) RETROCOMPUTING SYSTEMS GROUP\n");
...
static void runAndReport(int startIdx)
{
    ...
    t1 = clock();
    if (hadError)
        printf("\nEXECUTION TERMINATED ABNORMALLY.  CPU TIME: %.2f SEC\n",
               (double)(t1 - t0) / (double)CLOCKS_PER_SEC);
    else
        printf("\nEXECUTION TERMINATED.  CPU TIME: %.2f SEC\n",
               (double)(t1 - t0) / (double)CLOCKS_PER_SEC);
